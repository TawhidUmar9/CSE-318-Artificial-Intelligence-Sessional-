...
public:
    ...
    Board(int r, int c) : rows(r), cols(c), last_move_coords({-1, -1})
    {
        ...
        while (!explosion_queue.empty())
        {
            ...
            while (next_state.grid[current_r][current_c].count >= critical_mass)
            {
                ...
                for (int i = 0; i < 4; ++i)
                {
                    ...
                    if (neighbor_r >= 0 && neighbor_r < next_state.rows && neighbor_c >= 0 && neighbor_c < next_state.cols)
                    {
...
int simple_heuristic(const Board &state, char maximizing_player_color)
{
    ...
    for (int r = 0; r < state.rows; ++r)
        for (int c = 0; c < state.cols; ++c)
        {
            ...
            if (cell.color == player_enum)
                player_orbs += cell.count;
            else if (cell.color == opponent_enum)
...
std::pair<std::pair<int, int>, int> minimax(Board state, int depth, char maximizing_player_color, int alpha, int beta, int (*heuristic_func)(const Board &, char))
{
    ...
    if (current_player_on_turn == maximizing_player_color)
    {
        ...
        if (child_states.empty())
            return {{-1, -1}, INF_NEG_VAL}; // Current player has no moves, this path is bad
    ...
    { // Minimizing Player's Turn
        ...
        if (child_states.empty())
            return {{-1, -1}, INF_POS_VAL}; // Opponent has no moves, this path is good for us
